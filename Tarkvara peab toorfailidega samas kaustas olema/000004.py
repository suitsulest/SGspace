...
for i in range(176):
    ...
    if len(files) > 0:
        for x in range(0,len(files)):
            ...
            A = image.reshape(rows,cols)
            

            F  = A.reshape(cols,rows)
            ...
            image2 = cv2.cvtColor(rgb, cv2.COLOR_YCrCb2BGR)

                # save output uncorrected
            fname = files[x].split('.')
            cv2.imwrite(fname[0] + "U.tif", image2)
            result = cv2.resize(image2, dsize=(int(cols),int(rows)), interpolation=cv2.INTER_CUBIC)
